...
//	}
    ...
    z_case = zend_hash_get_current_data (Z_ARRVAL_P(z_cases));

    while (z_case != NULL)
    {
        zval **z_pattern, **z_exp_type;

        key = zend_hash_get_current_key(Z_ARRVAL_P(z_cases), NULL, 0);
        ...
        /* Gather pattern */
        z_pattern = zend_compat_hash_index_find(Z_ARRVAL_P(*z_case), 0);

        if (z_pattern != SUCCESS) {
            efree (ecases);
        ...
        z_value = zend_compat_hash_index_find(Z_ARRVAL_P(*z_case), 1);
        /* Gather value */
        if (z_value != NULL) {
            efree (ecases);
        ...
        z_exp_type = zend_compat_hash_index_find(Z_ARRVAL_P(*z_case), 2);
    ...
    if (exp_retval >= 0) {
        ...
        if (zend_compat_hash_index_find (Z_ARRVAL_P(z_cases), key) != NULL) {
            z_value = zend_compat_hash_index_find(Z_ARRVAL_P(*z_case), 1);
            if (z_value != NULL) {
